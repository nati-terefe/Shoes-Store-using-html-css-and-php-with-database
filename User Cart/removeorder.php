<?php 
    ...
    { // Function to register an admin
        ...
        if ($con) { // Connection successfully established
            $existorderquery = "SELECT * FROM orders WHERE Num=$id"; // Query to select rows with matching credentials
            ...
            if (!$numofexisting > 0) { // Selecting if theres no an already existing admin match
                ...
                $removequery = "delete from orders where Num=$id"; 
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") { // Checking for and POST Request to server
            ...
            if (
                ...
                echo "Removal of order is successful. Refresh Page to see change"; // Notifying Sign Up went successfully
